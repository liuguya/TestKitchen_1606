...
class CBSubjectCell: UITableViewCell {
    ...
    func showData(){
        ...
        if dataArray?.count > 2{
            let titleModel = dataArray![2]
            if titleModel.type == "text"{
                descLabel.text = titleModel.content!
            }
    ...
    class func CBSubjectCellFor(tableView: UITableView, atIndexPath indexPath: NSIndexPath, withListModel listModel: CBRecommendWidgetListModel) -> CBSubjectCell {
        ...
        if listModel.widget_data?.count > indexPath.row*3+2{
            ...
            cell?.dataArray = array.subarrayWithRange(NSMakeRange(indexPath.row*3, 3)) as? Array<CGRecommendWidgetDataModel>
